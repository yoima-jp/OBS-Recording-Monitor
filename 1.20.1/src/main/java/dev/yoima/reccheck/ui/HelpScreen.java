...
public final class HelpScreen extends Screen {
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		graphics.drawString(font, title, panelLeft + CONTENT_SIDE_PADDING, PANEL_MARGIN_TOP + 6, 0xFFFFFFFF, false);
		...
		graphics.drawString(font, Component.translatable("screen.reccheck.help.images"), contentLeft, y, 0xFFFFFFFF, false);
		y += SECTION_SPACING;

		for (HelpImage image : images) {
			graphics.drawString(font, Component.translatable(image.labelKey()), contentLeft, y, 0xFFB7C4D6, false);
			...
			graphics.blit(image.texture(), drawX, y, 0.0F, 0.0F, drawWidth, drawHeight, image.width(), image.height());
			y += drawHeight + CARD_PADDING * 2 + IMAGE_SPACING;
		}

		graphics.drawString(font, Component.translatable("screen.reccheck.help.details"), contentLeft, y, 0xFFFFFFFF, false);
		y += SECTION_SPACING;
		for (int i = 0; i < lines.size(); i++) {
			graphics.drawString(font, lines.get(i), contentLeft, y, 0xFFB7C4D6, false);
			y += 10;
		}
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
		scroll = Math.max(0, Math.min(scroll + (int) (-amount * 12), maxScroll));
		return true;
	}
	...
	private List<HelpImage> buildImages() {
		...
		result.add(new HelpImage(texture(prefix + "1"), "screen.reccheck.help.step1", japanese ? 904 : 964, japanese ? 373 : 335));
		result.add(new HelpImage(texture(prefix + "2"), "screen.reccheck.help.step2", japanese ? 671 : 659, japanese ? 329 : 337));
		result.add(new HelpImage(texture(prefix + "3"), "screen.reccheck.help.step3", japanese ? 437 : 434, japanese ? 166 : 204));
		return result;
	}

	private static ResourceLocation texture(String name) {
		return new ResourceLocation("reccheck", "textures/help/" + name + ".png");
	}
	...
	private record HelpImage(ResourceLocation texture, String labelKey, int width, int height) {
	}
