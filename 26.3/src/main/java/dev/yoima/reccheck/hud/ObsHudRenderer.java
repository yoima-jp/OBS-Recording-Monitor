...
public final class ObsHudRenderer implements HudElement {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, DeltaTracker tickCounter) {
		...
		renderOverlay(client, client.gui.screen(), graphics);
	}

	public void renderOverlay(Minecraft client, Screen screen, GuiGraphicsExtractor graphics) {
		if (client == null) {
			return;
		}
		...
		graphics.text(client.font, icon, iconX(client, x, icon), iconY(client, y), 0xFFFFFFFF, false);

		graphics.text(client.font, headline, x + TEXT_X, y + HEADLINE_Y, 0xFFFFFFFF, false);
		graphics.text(client.font, detail, x + TEXT_X, y + DETAIL_Y, 0xFFd7d7d7, false);

		if (!hint.getString().isEmpty()) {
			graphics.fill(x + TEXT_X, y + HINT_BOX_Y, x + drawWidth - 6, y + drawHeight - 4, 0x66000000);
			graphics.text(client.font, hint, x + TEXT_X + 4, y + HINT_TEXT_Y, 0xFFFFD35A, false);
		}
	}
	...
	private static void renderLiteHud(GuiGraphicsExtractor graphics, int width, int height, float scale, HudAnchor anchor, ObsConnectionSnapshot snapshot) {
	...
	private static void drawCircle(GuiGraphicsExtractor graphics, int x, int y, int diameter, int color) {
